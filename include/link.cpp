...
#include <stdarg.h>
	...
	#define DSETSOCKOPTPARAM (const char*)
	#pragma comment(lib,"ws2_32.lib")
#else
	#include <sys/socket.h>
	#include <netdb.h>
	#define DSETSOCKOPTPARAM (void*)
#endif
...
void Link::nodelay(bool enable){
	int opt = enable? 1 : 0;
	::setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, DSETSOCKOPTPARAM &opt, sizeof(opt));
}

void Link::keepalive(bool enable){
	int opt = enable? 1 : 0;
	::setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, DSETSOCKOPTPARAM &opt, sizeof(opt));
}

void Link::noblock(bool enable){
	noblock_ = enable;
	u_long ulBlock = enable ? 1 : 0;
...
Link* Link::listen(const char *ip, int port){
	...
	if(::setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, DSETSOCKOPTPARAM &opt, sizeof(opt)) == -1){
		goto sock_err;
	}
...
Link* Link::accept(){
	...
	int ret = ::setsockopt( client_sock, SOL_SOCKET, SO_LINGER, DSETSOCKOPTPARAM &opt, sizeof(opt));
	if (ret != 0) {
...
int Link::write(){
	...
	while((want = output->size()) > 0){
		...
		//want = 1;
#if defined(_WIN32)
		int len = ::send( (SOCKET) sock, output->data(), want, 0 );
#else
